...
char * get_list_message(){
	...
	for(index = bt_info_list.used -1; index > -1; index --){
		...
		else printed_chars += snprintf((list_message + printed_chars), (message_size - printed_chars) +1, "%s,", bt_info_string);
	...
	printf("Done. Message size: %i %s\n", message_size,list_message);
	return list_message;
...
void insert_hci_in_list(char *detected_mac, char *host_mac, int dbm_signal,char *address_type, char *dev_class){
	...
	BSON_APPEND_INT64(bt_info, "timestamp", (uint64_t)time(NULL));
	BSON_APPEND_UTF8(bt_info, "clase", dev_class);
